...
2.  .the **View(data)** function is used to open a separate window or pane to display the contents of an object, typically a data frame. It provides a convenient way to visually inspect and explore the data.
...
9.creating a contingency table named **tab2**. The **table()** function is used to generate a cross-tabulation of the factors **as.factor(data\$Renewed.)** and **data\$Years.of.No.Claims.Bonus**. The **dnn** parameter is used to assign names to the dimensions of the table.
